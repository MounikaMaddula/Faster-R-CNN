...
def expand(image, boxes, filler):
    ...
    new_image = torch.ones((3, new_h, new_w)) * filler.unsqueeze(1).unsqueeze(1)  # (3, new_h, new_w)
    new_image = new_image.type(torch.FloatTensor)
...
def random_crop(image, boxes, labels):
    ...
    while True:
        ...
        for _ in range(max_trials):
            ...
            #print (crop.shape, boxes.shape)
            overlap = IoU_area(crop.unsqueeze(0),boxes)  # (1, n_objects), n_objects is the no. of objects in this image
...
def resize(image, boxes, dims=(800, 800), return_percent_coords=False):
    """
